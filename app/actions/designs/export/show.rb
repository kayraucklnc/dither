# frozen_string_literal: true

module Dither
  module Actions
    module Designs
      module Export
        # The show action.
        class Show < Action
          config.formats.accept :zip

          include Deps["aspects.designs.exporter", repository: "repositories.screen_template"]

          using Refines::Actions::Response

          params { required(:design_id).filled :integer }

          def handle request, response
            parameters = request.params

            halt :unprocessable_content unless parameters.valid?

            extension = repository.find parameters[:design_id]

            case exporter.call extension
              in Success(body) then response.with body: body.read
              in Failure(message) then response.with body: message, status: 500
            end
          end
        end
      end
    end
  end
end
