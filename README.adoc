...
:devices_link: link:https://github.com/kayraucklnc/dither/blob/main/doc/devices.adoc[Devices]
:doc_api_link: link:https://github.com/kayraucklnc/dither/blob/main/doc/api.adoc[API]
:doc_configuration_link: link:https://github.com/kayraucklnc/dither/blob/main/doc/configuration.adoc[Configuration]
:doc_development_link: link:https://github.com/kayraucklnc/dither/blob/main/doc/development.adoc[Development]
:doc_docker_link: link:https://github.com/kayraucklnc/dither/blob/main/doc/docker.adoc[Docker]
:doc_extensions_link: link:https://github.com/kayraucklnc/dither/blob/main/doc/extensions.adoc[Extensions]
:doc_jobs_link: link:https://github.com/kayraucklnc/dither/blob/main/doc/jobs.adoc[Jobs]
:doc_kubernetes_link: link:https://github.com/kayraucklnc/dither/blob/main/doc/kubernetes.adoc[Kubernetes]
:doc_raspberry_pi_link: link:https://github.com/kayraucklnc/dither/blob/main/doc/raspberry_pi.adoc[Raspberry PI]
:doc_render_link: link:[link:https://github.com/kayraucklnc/dither/blob/main/doc/render.adoc[Render]]
...
Dither is a {ruby_link}/{hanami_link} web server for managing e-ink displays on your own
network. It talks the same firmware API as TRMNL hardware, renders screens from HTML, and
can flash boards over USB straight from the dashboard.

[NOTE]
====
Dither is a fork of link:https://github.com/usetrmnl/terminus[Terminus] by TRMNL, used
under the MIT licence; the original copyright notice is retained in `LICENSE.adoc`.

Dither is the server only. It speaks the same device API as TRMNL hardware, so boards run
**stock** link:https://github.com/usetrmnl/trmnl-firmware[trmnl-firmware] (GPL-3.0) with no
changes -- point one at this server and it provisions itself. Screens render against Dither's
own framework (`lib/dither/screen_framework.css`), so nothing off-origin is on the render
path. The one remaining call to TRMNL infrastructure is downloading official firmware
releases, which is the point rather than a leftover.
====

Dither is a {ruby_link}/{hanami_link} web server for running e-ink displays on your own network. It implements the {trmnl_link} device API, so stock hardware and stock firmware work against it unchanged, with no subscription and nothing phoning home. Where it diverges from upstream it does so on purpose: screens are composed from extensions rather than hand-written, and what a panel shows is decided by rules rather than a fixed playlist.
...
To immediately spin up Dither on your local machine using {docker_link}, run:

[source,bash]
----
curl https://raw.githubusercontent.com/kayraucklnc/dither/refs/heads/main/scripts/docker/quick.sh | bash
----
...
In accordance with the {unbrickable_pledge_link}, the latest version of Dither always supports the latest version of our {firmware_link}. If you are running on older firmware, you'll need to use an older version of Dither. Here's a rough breakdown of the Firmware version history where changes were applied that might cause issues if you haven't upgraded your devices in a while:

* *Firmware 1.6.9* (and lower): Supported by Dither link:https://github.com/kayraucklnc/dither/releases/tag/0.41.0[0.41.0] and lower.
* *Firmware 1.6.10* (and higher): Supported by Dither link:https://github.com/kayraucklnc/dither/releases/tag/0.42.0[0.42.0] and higher.
...
link:https://docs.google.com/forms/d/1FLVa9I1u6wk6ExMYj-l9P9a_Kw7XGXCPPyC0SSvv3hs/viewform[Take our survey] to help us learn more about how you use Dither along with any feedback you might have.
