...
under the MIT licence; the original copyright notice is retained in `LICENSE.adoc`. The
paired firmware is a fork of link:https://github.com/usetrmnl/trmnl-firmware[trmnl-firmware]
(GPL-3.0). Upstream references to `trmnl.com` that remain in this codebase are deliberate:
they are live interop endpoints (the extension rendering framework, the recipe gallery, the
palette/model APIs), not branding.
====
